...
void LogProcess(const LPSTR process, DWORD PSize, DWORD MCount, DWORD BAddr);

void PrintProcesses( DWORD processID );
void VerifyProcesses(const char *process, DWORD PSize, DWORD ModuleCount, DWORD BaseAddress);
bool StoredProcess(const char *process);

char ModuleList[MAX_MODS][MAX_PATH];
char ProcessList[MAX_PROCS][MAX_PATH];

void CheckVersionFile(void);
...
//		ModInfo = new MODULEINFO;
		...
		debuglog("PrintModules(), lpBaseOfDll = 0x%x (%d), SizeOfImage = %d, EntryPoint = 0x%x", 
			ModInfo->lpBaseOfDll, ModInfo->lpBaseOfDll, ModInfo->SizeOfImage, ModInfo->EntryPoint);
		...
		debuglog("PrintModules(), Process %s has %d modules", szProcessName, ModCount, ModInfo->lpBaseOfDll);

		VerifyProcesses(szProcessName, ModInfo->SizeOfImage, ModCount, (DWORD)ModInfo->lpBaseOfDll);
...
//		debuglog("Process %s was not found in list!", l_buff);
		...
		{
			LogProcess(l_buff, PSize, ModuleCount, BaseAddress);
		}
	}
}
...
/*
	...
	fopen_s(&version_fp, VERSION_LOG, "r");
	if(version_fp == NULL)
	{
		debuglog("Unable to open version log");
		VersionHack = TRUE;
		LogTweak("Version File Corrupted");
	}

	fseek(version_fp, SEEK_SET, 0);
	fgets(String, 1024, version_fp);
	debuglog("String = %s", String);
